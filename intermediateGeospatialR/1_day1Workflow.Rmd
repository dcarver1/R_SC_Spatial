...
```{r block1, echo = FALSE}
knitr::include_graphics("intermediateGeospatialR/data/pngsForMarkdown/light.jpg")
...
Since this lesson is in an RProject, your working directory should be automatically set to the correct path. Double check that it is set to something that looks like: `~/Desktop/R_SC_Spatial/`
...
# I typically do this by running single lines at a time but here we are just 
# commenting out sections of the process we don't want to run 
#for(i in seq_along(counties$STATEFP)){
  c1 <- counties[i,] # select a specific county 
#  for(j in seq_along(images)){
    ## pull a specific month's image 
...
**Connect Both Variables in Simplified Structure**
...
r1 <- raster::raster("intermediateGeospatialR/data/nightLights/Harris/june_10arc.tif")
...
sf::write_sf("intermediateGeospatialR/outputs/censusNightLightRadiance.shp")
```
