...
In this section we will be building a RMD document that takes the methods we developed in the previous sections and puts them into a framework that will become a reproducible work flow. 

At the end of this section, you will have  rmd file that has flexible input paraments, so it can be called to produce maps and figure at various time frames and locations 

The last past of this lesson we will create an R script that we can used to iterate over multiple parameters to produce multiple summary documents. 
...
The indexing and geoprocessing within the loop of the months is nothing new. It is worth noting that we are creating a new dataframe each month rather then outside of the initial loop. This is not the only way to do it, but I find it to be the easiest way. This is hard to visualize at the moment but I'll just say that if we moved the dataframe initialization outside the first loop we would need to change how we store the data we generate. 
...
Much like before gleaming these conclusion from the table is possible but if visualized well they will jump off the page. 
<br>
