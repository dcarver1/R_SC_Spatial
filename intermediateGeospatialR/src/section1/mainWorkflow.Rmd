...
In the example provided, we can see that the distribution of radiance values is right skewed. This means that the majority of the observations are at or near zero and there are a few locations that significantly brighter. The is important information to understand before starting the analysis as most of the observations will tell us little more then it is dark at that location.

*HP: What example provided?* 
...
`tidycensus` requires a personal access token to utilize the API. As such, we are not going to be pulling data from tidy census as part of this lesson. However, the code showing how the data was pulled is provided as supporting material if this is something you need to do later on in your work.
...
`dplyr` is a great library for efficiently performing multiple operations on a dataset. We will be using it for a lot of things. 
...
Now that we know that process works, let's try to streamline this process a little bit so that we're not creating so many variables. We will utilize `dplyr` piping structure. The `%>%` pipe is a custom operator that takes the output of one function and places it as an input into another function. 
...
```{r}
source(paste0(baseDir, "/src/section1/clipMask2.R"))
print(clipMask2)
```
...
What do ".x" and ".y" mean in this dataset? I already forgot. Rename the variable before going any further. 
...
# View(t2)
```
*HP: First comment in above code block is missing a word I think.*
...
```{r}
## raster is awesome, the extract function is all we need to summarize radiance over a spatial feature
t2$june <- raster::extract(x = r1, y = t2, fun = mean) ## reduce the 
```
*HP: Reduce the what? (Comment in code above.)*
...
matches <- grepl(pattern = id, x = c3$GEOID)
```

#! grepl has already been used. the explaination here might not be need or can be moved to up above. 

*HP: I think how you have it now looks good. People tend to get a little confused with pattern matching, so repeating the explanation a bit is useful.*
...
for(i in seq_along(counties$STATEFP)){
  ...
  c1 <- c3[grepl(pattern = id, x = c3$GEOID),][1:3, ] ### we're only looking at the first few records because this will take a long time to run. 1,200 census tracks, 12 images = 14,400 spatial operations. Pretty sweet! Aren't you glad you're not clicking through this workflow :) 
...
```
*HP: Extra what? (Third comment in code above.)*
...
# View(df)
```
*HP: Same as before--extra what? (Third comment in code above.)*
...
sf::write_sf(paste0(baseDir,"/outputs/censusNightLightRadiance.shp"))
```
...
```{r}
### ee now have an average yearly radiance for each location, so let's plot the relationship against radiance and see what we see.
library(ggplot2)
